...
//string thisValue = (string)w[theKey];  
	...
	void word_latter () {
		for(int i=0; i<Main.letter; i++)
		{
			...
			{
				CreateButton(_parent,(-80*(Main.letter-1))/2+190f*i,-100f, 1f, 1f , "", i);
			}
			else if(Main.word.Length<13)
			{
				CreateButton(_parent,((-80*(Main.letter-1))/2+90f*i)*0.95f,-100f, 0.95f, 0.95f , "", i);	
			}
			else
			{
				CreateButton(_parent,((-80*(Main.letter-1))/2+90f*i)*0.85f,-100f, 0.85f, 0.85f , "", i);	
			}
