...
public class Buttons : Main {
	...
	void Start () 
	{
		...
		if (PlayerPrefs.GetInt ("sound") == 1 && gameObject.name == "sound") {
			...
			if(gameObject.name=="WFW"){
				if( PlayerPrefs.GetInt("WBonus") ==1)
					gameObject.SetActive(false);
				else{	
					BonusInfo = GameObject.Find("BonusInfo");
	...
	void OnPress (bool isDown)
	{
		if (isDown == false)
		{
			...
			if(gameObject.name=="pack1")Main.onBuy("wfw2_p1");
			if(gameObject.name=="pack2")Main.onBuy("wfw2_p2");
			if(gameObject.name=="pack3")Main.onBuy("wfw2_p3");
			if(gameObject.name=="pack4")Main.onBuy("wfw2_p4");
			if(gameObject.name=="pack5")Main.onBuy("wfw2_p5");	
	...
	{
		if( PlayerPrefs.GetInt("WBonus") !=1 && gameObject.GetComponent<UIInput>().value.ToLower()=="professional")
		{
			PlayerPrefs.SetInt("Coin", PlayerPrefs.GetInt("Coin")+3000);
			PlayerPrefs.SetInt("WBonus", 1);
			GameObject.Find("Camera").SendMessage("onCoin");
			GameObject.Find("WFW").SetActive(false);

			
		}else{
			GameObject.Find("BomusLabel").GetComponent<UILabel>().text = "Вы ввели ["+ gameObject.GetComponent<UIInput>().value +"], это не правильное слово!";
			gameObject.GetComponent<UIInput>().value = "Знаю слово!";
		}
	}
